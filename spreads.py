...
class TarotSpread:
    ...
    Оформи ответ строго по шаблону ниже. Каждый пункт должен быть отдельным блоком.

    🔮 ОБЩАЯ АТМОСФЕРА:
    (2-3 предложения об общей энергии расклада, как карты связаны между собой)

    📍 РАЗБОР КАЖДОЙ ПОЗИЦИИ:
    Для каждой карты из списка выше объясни:
    - Что эта карта говорит в контексте вопроса
    - Как её значение связано с конкретной позицией
    - Какие эмоции/мысли/действия она описывает

    💡 ГЛАВНОЕ ПОСЛАНИЕ:
    (одна ключевая мысль, которую клиент должен понять из расклада)

    🌟 ПРАКТИЧЕСКИЙ СОВЕТ:
    (конкретные действия на основе карт, на что обратить внимание)

    ✨ ПОЖЕЛАНИЕ:
    (вдохновляющая фраза в конце)
