...
# Создаем колоду для использования в bot.py (если нужно)
deck = TarotDeck()
CARDS = deck.cards  # Теперь CARDS - это список объектов
...
class TarotSpread:
    ...
    def __init__(self, openai_api_key: str = None):
        """
        Инициализация с OpenAI API ключом
        """
        ...
        # Создаем клиента OpenAI (правильный способ для версии 1.x)
        self.client = openai.OpenAI(
    api_key=openai_api_key,  # Ваш ключ OpenAI
    base_url="https://gptunnel.ru/v1"  # Адрес GPTunnel
)
    ...
    def _get_cards_from_names(self, card_names: List[str]) -> List[Dict]:
        ...
        for name in card_names:
            for card in CARDS:
                if card["name"] == name:
                    # Добавляем случайное положение (прямое/перевернутое)
                    card_copy = card.copy()
                    card_copy["position"] = random.choice(["прямое", "перевернутое"])
                    cards_data.append(card_copy)
                    break
        return cards_data
    ...
    async def generate_reading(self, 
        ...
        Args:
            ...
            model: Модель OpenAI (по умолчанию gpt-4o-mini - дешевая)
        """
...
"""
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                temperature=0.8,  # Немного креативности для разнообразия
                max_tokens=1000
            )
