...
function displayResult(data) {
    ...
    html += '<h2 style="color:#8a6bff;">✅ Данные получены!</h2>';
    html += '<p>Статус: ' + (data.status || 'нет статуса') + '</p>';
    
    if (data.interpretation) {
        html += '<p>Длина ответа: ' + data.interpretation.length + '</p>';
        html += '<div style="background:rgba(0,0,0,0.3); padding:15px; border-radius:10px; margin-top:15px; text-align:left;">';
        html += data.interpretation.substring(0, 200) + '...';
        html += '</div>';
    } else {
        html += '<p style="color:red;">❌ Нет поля interpretation в ответе!</p>';
        html += '<p>Полный ответ: ' + JSON.stringify(data) + '</p>';
    }
    
    html += '<button onclick="window.location.reload()" style="margin-top:30px; padding:15px 30px; background:#8a6bff; border:none; border-radius:10px; color:white;">Назад</button>';
    html += '</div>';
    
    content.innerHTML = html;
    console.log("✅ Тестовый результат отображен");
}
